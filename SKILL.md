...
version: 1.0.4
summary: 一键把 Web 端当前登录用户「收藏」tab 的收藏笔记批量导出为本地 Markdown（每篇含标题、原文链接、正文、图片/视频 URL），支持 offset/count 分批断点续导，断点可重跑不重复。不需要任何第三方 API key 或付费额度，用你 Chrome 里已登录的账号跑。当用户需要导出/备份/检索自己的收藏内容、想离线保存收藏笔记、建个人知识库时唤起。
description: 一键把 Web 端当前登录用户「收藏」tab 的收藏笔记批量导出为本地 Markdown（每篇含标题、原文链接、正文、图片/视频 URL），支持 offset/count 分批断点续导，断点可重跑不重复。不需要任何第三方 API key 或付费额度，用你 Chrome 里已登录的账号跑。当用户需要导出/备份/检索自己的收藏内容、想离线保存收藏笔记、建个人知识库时唤起。
license: MIT
---
